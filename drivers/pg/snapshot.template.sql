...
--DROP TABLE %BASE_SCHEMA%.snapshot_reghosts;
CREATE TABLE %BASE_SCHEMA%.snapshot_reghosts (
	id	NUMERIC,
	ip	VARCHAR(30),
	mowner	VARCHAR(30),
	mname	VARCHAR(30),
...
CREATE UNIQUE INDEX snapshot_reghosts_uix ON %BASE_SCHEMA%.snapshot_reghosts (ip, mowner, mname);
CREATE SEQUENCE %BASE_SCHEMA%.snapshot_reghosts_id_seq;

CREATE OR REPLACE FUNCTION %BASE_SCHEMA%.snapshot_do (
	comm_key VARCHAR, 
	op VARCHAR, 
	m_owner VARCHAR,
	m_name VARCHAR,
  ...
  BEGIN
	...
	IF (op = 'ALLOW') THEN
		IF (current_user = 'postgres') THEN
			...
			INSERT INTO %BASE_SCHEMA%.snapshot_reghosts(id, ip, mowner, mname)
			VALUES (nextval('%BASE_SCHEMA%.snapshot_reghosts_id_seq'), client_ip, master_owner, master_name);
		ELSE
	...
	ipnum := %BASE_SCHEMA%.getGrantedIp(master_owner, master_name);
	...
	IF (op = 'REGISTER') THEN
		...
		BEGIN
			...
			FROM %BASE_SCHEMA%.pg_mlogs
				INNER JOIN %BASE_SCHEMA%.pg_slogs ON pg_mlogs.snaplogid=pg_slogs.snaplogid
		...
		IF (slog_id IS NULL) THEN
			...
			INSERT INTO %BASE_SCHEMA%.pg_slogs(snaplogid, snapid, snaptime, userid) 
			VALUES (slog_id,
				snap_id,
	...
	ELSIF (op = 'UNREGISTER') THEN
		...
		ELSE
			snap_id := (additional || ipnum)::NUMERIC;
		END IF;
		DELETE FROM %BASE_SCHEMA%.pg_slogs
		WHERE snaplogid IN (
			SELECT snaplogid 
			FROM %BASE_SCHEMA%.pg_mlogs 
			WHERE pg_mlogs.masterschema=master_owner 
	...
	ELSIF (op = 'UPDATE_NULL') THEN
		...
		EXECUTE 'UPDATE ' || master_owner || '.' || %BASE_SCHEMA%.snapshotlog_name(master_owner, master_name) || ' SET snaptime$$ = TO_TIMESTAMP(' || quote_literal(refreshedTime) || ', ''YYYY-MM-DD HH24:MI:SS'') WHERE snaptime$$ IS NULL ';
	ELSE
...
CREATE OR REPLACE FUNCTION %BASE_SCHEMA%.getGrantedIp (
	m_owner VARCHAR,
...
DECLARE
  ...
  BEGIN
	...
	IF (substr(inet_client_addr()::varchar, 1, length(inet_client_addr()::varchar)-3) <> '127.0.0.1') THEN
		...
		SELECT ip INTO host
		FROM %BASE_SCHEMA%.snapshot_reghosts snt
...
CREATE OR REPLACE FUNCTION %BASE_SCHEMA%.last_log_refresh (
	mowner VARCHAR,
	mname VARCHAR,
...
CREATE OR REPLACE FUNCTION %BASE_SCHEMA%.snapshotlog_exists (
	mowner VARCHAR,
...
DECLARE
  ...
  BEGIN
	...
	IF ( total > 0 ) THEN
		EXECUTE 'SELECT 1 from '||master_owner||'.' || %BASE_SCHEMA%.snapshotlog_name(master_owner, master_name) || ' WHERE 1=0';
		RETURN 'T';
	ELSE
		RETURN 'F';
	END IF;
...
CREATE OR REPLACE FUNCTION %BASE_SCHEMA%.snapshotlog_columns (
	mowner VARCHAR,
	mname VARCHAR ) RETURNS VARCHAR SECURITY DEFINER AS
$BODY$
DECLARE
  ...
  BEGIN
	...
	OPEN c_scolumns FOR
		SELECT *
		FROM %BASE_SCHEMA%.pg_mlog_refcols
		WHERE masterschema=master_owner
			AND mastername=master_name;
	LOOP
...
CREATE OR REPLACE FUNCTION %BASE_SCHEMA%.snapshotlog_name (
	mowner VARCHAR,
	mname VARCHAR ) RETURNS VARCHAR SECURITY DEFINER AS
$BODY$
DECLARE
...
CREATE OR REPLACE FUNCTION %BASE_SCHEMA%.count_log_modified_rows (
	mowner VARCHAR,
	mname VARCHAR,
...
DECLARE
  ...
  BEGIN
	...
	OPEN c_dynsql FOR EXECUTE 'SELECT count(*) as total FROM ' || master_owner || '.' || %BASE_SCHEMA%.snapshotlog_name(master_owner, master_name) || ' WHERE ((snaptime$$ > TO_TIMESTAMP(' || quote_literal(last_refresh) || ', ''YYYY-MM-DD HH24:MI:SS'')) OR (snaptime$$ IS NULL))';
...
CREATE OR REPLACE FUNCTION %BASE_SCHEMA%.snapshotlog_ud_filter (
...
CREATE OR REPLACE FUNCTION %BASE_SCHEMA%.snapshotlog_iu_filter (
	last_refresh VARCHAR) RETURNS VARCHAR SECURITY DEFINER AS
$BODY$
BEGIN
	RETURN '((snaptime$$ > TO_TIMESTAMP(' || quote_literal(last_refresh) || ',''YYYY-MM-DD HH24:MI:SS'')) OR (snaptime$$ IS NULL))
...
GRANT EXECUTE ON FUNCTION %BASE_SCHEMA%.snapshot_do(VARCHAR,VARCHAR,VARCHAR,VARCHAR,VARCHAR) TO PUBLIC;
GRANT EXECUTE ON FUNCTION %BASE_SCHEMA%.last_log_refresh(VARCHAR, VARCHAR, NUMERIC) TO PUBLIC;
GRANT EXECUTE ON FUNCTION %BASE_SCHEMA%.snapshotlog_exists(VARCHAR, VARCHAR) TO PUBLIC;
GRANT EXECUTE ON FUNCTION %BASE_SCHEMA%.snapshotlog_columns(VARCHAR, VARCHAR) TO PUBLIC;
GRANT EXECUTE ON FUNCTION %BASE_SCHEMA%.snapshotlog_name(VARCHAR, VARCHAR) TO PUBLIC;
GRANT EXECUTE ON FUNCTION %BASE_SCHEMA%.count_log_modified_rows(VARCHAR,VARCHAR,VARCHAR) TO PUBLIC;
GRANT EXECUTE ON FUNCTION %BASE_SCHEMA%.snapshotlog_ud_filter(VARCHAR) TO PUBLIC;
GRANT EXECUTE ON FUNCTION %BASE_SCHEMA%.snapshotlog_iu_filter(VARCHAR) TO PUBLIC;
